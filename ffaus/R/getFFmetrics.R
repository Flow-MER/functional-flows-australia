...
#' @param user_q10_thresh pass a user defined 10th percentile threshold for use in calculating exceedance metrics.
#' @param user_q25_thresh pass a user defined 25th percentile threshold for use in calculating exceedance metrics.
#' @param user_q50_thresh pass a user defined 50th percentile threshold for use in calculating exceedance metrics.                   
                         ...
                         mfrow_cl    = 2)
{
  ...
  if (!is.null(plot_out_base))
  {
    ...
    png(filename = file,
        bg = "white", width =  plot_width, height = plot_height, 
        units = 'in', res = 300)
      par(mfrow = c(mfrow_rw, mfrow_cl))
      all_year_out <- calcAllYear(flow_matrix, do_plot = do_plot, 
                                  user_q10_thresh = user_q10_thresh, 
                                  user_q25_thresh = user_q25_thresh,
                                  user_q50_thresh = user_q50_thresh, 
  ...
  out_all_year$All_Years_Avg <- c(round(c(mean(flow_matrix$Flow, na.rm = T), 
                                          stats::sd(flow_matrix$Flow, na.rm = T),
                                          stats::sd(flow_matrix$Flow, na.rm = T) / mean(flow_matrix$Flow, na.rm = T),
                                          all_year_out$mags_global, 
                                          floor(rowMeans(all_year_out$tims,  na.rm = T)),
                                          ...
                                          function(x) {names(which.max(table(as.character(x), useNA = "always")))[1]}))),
  ...
  if (!is.null(plot_out_base))
  {
    ...
    file      <- paste0(plot_out_base, "_", plot_type, ".png")
    png(filename = file,
        bg = "white", plot_width, height = plot_height, 
        units = 'in', res = 300)
      par(mfrow = c(mfrow_rw, mfrow_cl))
